...
def top_entities(topic_id: int, topic_model: object, topics: list, docs_embedding, data_news, entities: list, n_entities=10):
    ...
    try:
        # Obtener ID y pos_relativa de documentos de un topico ordenados de mayor a menor prob
        ...
        s_coseno = []
        for i in df_query["pos_rel"]:
            s_coseno.append(cosine_similarity([topic_model.topic_embeddings_[topic_id + 1]], [docs_embedding[i]])[0][0])

        df_query['similitud'] = s_coseno
        
        # umbral
        threshold = df_query['similitud'].mean()

        # Ordenado por mayor similitud
        df_filtered = df_query[df_query["similitud"] > threshold].sort_values("similitud", ascending=False)

        # Entidades de documentos ordenados para el topico elelgido (cantidad por documento=n_entities)
        entities_topic = []
        for doc in list(df_filtered["pos_rel"]):
            entities_topic.append(entities.iloc[doc][:n_entities])
